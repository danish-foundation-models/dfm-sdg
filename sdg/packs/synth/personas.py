...
from sdg.packs.synth.query_profiles import load_weighted_query_profiles
from sdg.packs.synth.record_loader import load_records_source
from sdg.packs.synth.types import Persona, QueryPlan, Record
...
def load_personas(cfg: Record, *, family: str = "memorization") -> list[Persona]:
...
def build_query_plans(
    ...
    *,
    ...
    return list(iter_query_plans(fact_bundles, cfg, family=family, seed=seed))
...
def iter_query_plans(
    ...
    *,
    ...
    personas = load_personas(cfg, family=family)
    assert personas, f"No personas available for {family} generation"

    rng = Random(0 if seed is None else seed)
    rng.shuffle(personas)

    query_angles = _query_angles(cfg, family=family)
    query_profiles = load_weighted_query_profiles(cfg, family=family, seed=seed)
    assistant_style = load_assistant_style(cfg, family=family)
...
def _query_angles(cfg: Record, *, family: str) -> list[str]:
    ...
    raw_angles = family_cfg.get("query_angles", DEFAULT_QUERY_ANGLES)
