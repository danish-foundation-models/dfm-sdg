...
def row_evidence_tokens(row: dict[str, Any]) -> set[str]:
    evidence_tokens = set(meaningful_tokens(row["hidden"]["sentence"]))
    teacher_bundle = row["hidden"].get("teacher_bundle") or {}

    for sentence in teacher_bundle.get("supporting_claims", []):
        evidence_tokens.update(meaningful_tokens(sentence))
    for item in teacher_bundle.get("structured_context", []):
        evidence_tokens.update(meaningful_tokens(item))
    for item in teacher_bundle.get("retrieved_claims", []):
        evidence_tokens.update(meaningful_tokens(item))
    for item in teacher_bundle.get("retrieved_context", []):
        evidence_tokens.update(meaningful_tokens(item.get("snippet", "")))
    for source in row.get("sources", []):
        evidence_tokens.update(meaningful_tokens(source.get("snippet", "")))
...
def row_retrieval_grounded(row: dict[str, Any]) -> bool:
    ...
    teacher_bundle = row["hidden"].get("teacher_bundle") or {}
    teacher_tokens = set(meaningful_tokens(teacher_bundle.get("primary_claim", "")))
    for sentence in teacher_bundle.get("supporting_claims", []):
        teacher_tokens.update(meaningful_tokens(sentence))
    for item in teacher_bundle.get("structured_context", []):
        teacher_tokens.update(meaningful_tokens(item))

    retrieved_tokens = set()
    for source in sources:
        retrieved_tokens.update(meaningful_tokens(source.get("snippet", "")))
...
def row_reasoning_grounded(row: dict[str, Any]) -> bool:
    reasoning_tokens = meaningful_tokens(row.get("reasoning", ""))
    ...
    target_tokens = meaningful_tokens(row.get("target", ""))
...
def row_coverage_supported(row: dict[str, Any]) -> bool:
    ...
    answer_tokens = meaningful_tokens(row.get("target", ""))
    if not answer_tokens:
        return False

    supported_points = 0
    for point in coverage_points:
        point_tokens = meaningful_tokens(point)
        if not point_tokens:
            continue
...
def row_answer_supported(row: dict[str, Any]) -> bool:
    ...
    source_title_tokens = meaningful_tokens(row["hidden"].get("source_title", ""))
